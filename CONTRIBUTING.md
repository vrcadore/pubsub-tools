...
2. Clone the **upstream** repository to local:
   ```bash
   git clone https://github.com/vrcadore/pubsub-tools.git
   # Or if you prefer SSH clone:
   git clone git@github.com:vrcadore/pubsub-tools.git
   ```
3. Add the fork as a new remote:
   ```bash
   git remote add fork https://github.com/yourname/pubsub-tools.git
   git fetch fork
   ```
...
You will need to install base dependencies in a venv. Make sure your `pip` is newer than `21.3` to install PubSub Tools in develop/editable mode.
